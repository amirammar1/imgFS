...
struct imgfs_header {
    char name[MAX_IMGFS_NAME];                         // Name of the database
    uint32_t version;                                  // Version of the database
    uint32_t nb_files;                                 // Current number of images
    uint32_t max_files;                                // Maximum number of images the system can contain
    uint16_t resized_res[2 * (NB_RES - 1)];            // Resolutions of thumbnail and small images
    uint32_t unused_32;                                // Unused 32-bit unsigned int
    uint64_t unused_64;                                // Unused 64-bit unsigned int
};

struct img_metadata {
    char img_id[MAX_IMG_ID+1];                           // Unique identifier for the image
    unsigned char SHA[SHA256_DIGEST_LENGTH];           // Image hash code (SHA-256)
    uint32_t orig_res[2];                              // Resolution of the original image
    uint32_t size[NB_RES];                             // Memory sizes (in bytes) of images at different resolutions
    uint64_t offset[NB_RES];                           // Positions in the image database file for images at different resolutions
    uint16_t is_valid;                                 // Indicates whether the image is in use
    uint16_t unused_16;                                // Not used, reserved for future use
};

struct imgfs_file {
    FILE* file;                                        // File containing everything
    struct imgfs_header header;                               // Header of the image database
...
/**
 * @brief Check if the open mode is valid or not
 *
 * @param mode Mode to check
 * @return 0 if the open mode is valid and 1 otherwise
 */
